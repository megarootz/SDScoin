...
uint64_t Currency::baseRewardFunction(uint64_t alreadyGeneratedCoins, uint32_t height) const {
  ...
  if (alreadyGeneratedCoins == 0) {
    ...
    std::cout << "Genesis block reward: " << base_reward << std::endl;
  }else{
...
difficulty_type Currency::nextDifficulty(std::vector<uint64_t> timestamps,
	...
	// Hardcode difficulty for 61 blocks after fork height - Not for coins that are starting
	...
	int64_t initial_difficulty_guess = 10000; // Dev needs to select this. Guess low.
